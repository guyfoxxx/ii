...
function buildLocalFallbackAnalysis(symbol, timeframe, candles, st, reason = "") {
  const tf = timeframe || "H4";
  const snap = computeSnapshot(Array.isArray(candles) ? candles : []);
  const data = Array.isArray(candles) && candles.length ? candles : [];
  const recent = data.slice(-Math.min(60, data.length));
  const hi = recent.length ? Math.max(...recent.map((x) => Number(x.h || x.c || 0))) : 0;
  const lo = recent.length ? Math.min(...recent.map((x) => Number(x.l || x.c || 0))) : 0;
  const last = snap?.lastPrice || (data.length ? Number(data[data.length - 1].c || 0) : 0);
  const span = Math.max(0, hi - lo);
  const mid = lo + span * 0.5;
  const r1 = lo + span * 0.786;
  const s1 = lo + span * 0.214;
  const trend = snap?.trend || "نامشخص";
  const bias = trend === "صعودی" ? "Buy on Dip" : (trend === "نزولی" ? "Sell on Rally" : "Range / Neutral");
  const stop = trend === "صعودی" ? Math.max(0, s1 - span * 0.08) : Math.max(0, r1 + span * 0.08);
  const tp1 = trend === "صعودی" ? r1 : s1;
  const tp2 = trend === "صعودی" ? hi : lo;
  const levels = [lo, s1, mid, r1, hi].filter((n) => Number.isFinite(n) && n > 0).map((n) => Number(n.toFixed(6)));

  return `⚠️ خروجی اضطراری (Fallback)
به‌دلیل اختلال موقت در سرویس مدل متنی، تحلیل با موتور داخلی تولید شد${reason ? ` (${reason})` : ""}.

1) وضعیت بازار (${symbol} / ${tf})
- قیمت فعلی: ${Number.isFinite(last) ? last : "نامشخص"}
...
3) سناریوی معامله (آموزشی)
- ورود پیشنهادی: ${trend === "صعودی" ? `ناحیه ${levels[1] || "-"} تا ${levels[2] || "-"}` : `ناحیه ${levels[2] || "-"} تا ${levels[3] || "-"}`}
- حد ضرر: ${Number.isFinite(stop) ? Number(stop.toFixed(6)) : "-"}
- تارگت 1: ${Number.isFinite(tp1) ? Number(tp1.toFixed(6)) : "-"}
- تارگت 2: ${Number.isFinite(tp2) ? Number(tp2.toFixed(6)) : "-"}

4) مدیریت ریسک
- ریسک انتخابی شما: ${st?.risk || "متوسط"}
- فقط با تایید کندل و بدون FOMO ورود بزن.

5) هشدار
- این خروجی جایگزین اضطراری است و باید با چارت زنده/کندل تایید شود.
- مسئولیت نهایی معامله با کاربر است.`;
}
...
async function runSignalTextFlowReturnText(env, from, st, symbol, userPrompt) {
  ...
  try {
    ...
    try {
      ...
      draft = buildLocalFallbackAnalysis(symbol, st.timeframe || "H4", candles, st, String(e2?.message || e2 || "text_providers_unavailable"));
    }
  }
...
async function verifyMiniappAuth(body, env) {
  ...
  const lenient = env.MINIAPP_AUTH_LENIENT;
  const res = await verifyTelegramInitData(initData, env.TELEGRAM_BOT_TOKEN, env.INITDATA_MAX_AGE_SEC, lenient);
  if (res.ok) return res;

  // super-lenient fallback for environments where Telegram WebApp initData is not reliably available
  if (String(lenient || "").trim() === "1" || String(lenient || "").toLowerCase() === "true") {
    const userIdRaw = body?.userId || body?.uid || body?.telegramUserId;
    const userId = Number(userIdRaw || 0);
    const username = String(body?.username || body?.handle || "").trim().replace(/^@/, "");
    if (userId > 0) return { ok: true, userId, fromLike: { username } };
  }
  return res;
}
...
async function api(path, body){
  ...
  const withAuth = {
    ...body,
    userId: body?.userId || unsafeUser?.id || 0,
    username: body?.username || unsafeUser?.username || "",
  };
