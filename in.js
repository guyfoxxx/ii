    ...
    if (String(env.QUICKCHART || "1") !== "0") {
      try {let candles = [];
        ...
        if (!Array.isArray(candles) || candles.length === 0) {
          ...
          const caption = `📈 چارت ${symbol} (${st.timeframe || "H4"})`;
          try {
            const res = await fetchWithTimeout(chartUrl, {}, Number(env.QUICKCHART_TIMEOUT_MS || 9000));
            if (!res.ok) throw new Error(`quickchart_http_${res.status}`);
            const mime = detectMimeFromHeaders(res, "image/png");
            const buf = await res.arrayBuffer();
            const b64 = arrayBufferToBase64(buf);
            const dataUrl = `data:${mime};base64,${b64}`;
            await tgSendPhoto(env, chatId, dataUrl, caption, kb([[BTN.HOME]]));
          } catch (e) {
            console.error("chart send error:", e);
            try {
              await tgSendPhoto(env, chatId, chartUrl, caption, kb([[BTN.HOME]]));
            } catch (err) {
              console.error("chart url send error:", err);
