...
/support | پشتیبانی
/miniapp | ورود مینی‌اپ
/quote BTCUSDT | قیمت لحظه‌ای
/news XAUUSD | اخبار
/newsanalyze EURUSD | تحلیل خبر
...
async function miniappBotToken(userId, env, ttlSec = 6 * 3600) {
  const now = Math.floor(Date.now() / 1000);
  const exp = now + Math.max(600, Number(ttlSec || 0));
  const payload = `${userId}.${exp}`;
  const secret = await hmacSha256Raw(utf8("MiniappBotAuth"), utf8(String(env.TELEGRAM_BOT_TOKEN || "miniapp_dev_secret")));
  const sig = await hmacSha256Hex(secret, utf8(payload));
  return `bot_${payload}.${sig}`;
}

async function buildMiniappUrlForUser(env, userId) {
  const base = getMiniappUrl(env);
  if (!base) return "";
  const t = await miniappBotToken(userId, env);
  const u = new URL(base);
  u.searchParams.set("initData", t);
  return u.toString();
}

function miniappInlineKeyboard(env, urlOverride = "") {
  const url = urlOverride || getMiniappUrl(env);
  if (!url) return null;
...
async function tgSetChatMenuButton(env, chatId, webAppUrl) {
...
async function handleUpdate(update, env) {
  try {
    ...
    if (text.startsWith("/quote ") || text.startsWith("/price ")) {
      const symbol = String(text.split(/\s+/)[1] || "").toUpperCase().trim();
      if (!symbol || !isSymbol(symbol)) return tgSendMessage(env, chatId, "نماد نامعتبر است. مثال: /quote BTCUSDT", mainMenuKeyboard(env));
      const tf = st.timeframe || "H4";
      try {
        const candles = await getMarketCandlesWithFallback(env, symbol, tf);
        const snap = computeSnapshot(candles || []);
        if (!snap) throw new Error("quote_bad_data");
        const cp = Number(snap.changePct || 0);
        return tgSendMessage(env, chatId, `📊 ${symbol} (${tf})
قیمت: ${snap.lastPrice}
تغییر: ${(cp > 0 ? "+" : "") + cp.toFixed(3)}%
روند: ${snap.trend || "نامشخص"}`, mainMenuKeyboard(env));
      } catch (e) {
        return tgSendMessage(env, chatId, "⚠️ دریافت قیمت فعلاً ممکن نیست. دوباره تلاش کنید.", mainMenuKeyboard(env));
      }
    }

    if (text.startsWith("/news ")) {
      const symbol = String(text.split(/\s+/)[1] || "").toUpperCase().trim();
      if (!symbol || !isSymbol(symbol)) return tgSendMessage(env, chatId, "نماد نامعتبر است. مثال: /news EURUSD", mainMenuKeyboard(env));
      try {
        const rows = await fetchSymbolNewsFa(symbol, env);
        const lines = (rows || []).slice(0, 5).map((x, i) => `${i + 1}) ${x.title || "-"}`);
        return tgSendMessage(env, chatId, `📰 اخبار ${symbol}

${lines.length ? lines.join("
") : "خبری یافت نشد."}`, mainMenuKeyboard(env));
      } catch {
        return tgSendMessage(env, chatId, "⚠️ دریافت اخبار فعلاً ممکن نیست.", mainMenuKeyboard(env));
      }
    }

    if (text.startsWith("/newsanalyze ")) {
      const symbol = String(text.split(/\s+/)[1] || "").toUpperCase().trim();
      if (!symbol || !isSymbol(symbol)) return tgSendMessage(env, chatId, "نماد نامعتبر است. مثال: /newsanalyze XAUUSD", mainMenuKeyboard(env));
      try {
        const rows = await fetchSymbolNewsFa(symbol, env);
        const summary = await buildNewsAnalysisSummary(symbol, rows, env);
        return tgSendMessage(env, chatId, `🧠 تحلیل خبر ${symbol}

${summary || "—"}`, mainMenuKeyboard(env));
      } catch {
        return tgSendMessage(env, chatId, "⚠️ تحلیل خبر فعلاً در دسترس نیست.", mainMenuKeyboard(env));
      }
    }
...
` : "");
    ...
    if (text === "/miniapp" || text === BTN.MINIAPP) {
      const url = await buildMiniappUrlForUser(env, st.userId || userId);
      if (!url) {
...
در Wrangler / داشبورد یک متغیر ENV به نام MINIAPP_URL یا PUBLIC_BASE_URL بگذار (مثلاً https://<your-worker-domain>/ ) و دوباره Deploy کن.`, mainMenuKeyboard(env));
      ...
      await tgSetChatMenuButton(env, chatId, url);
      return tgSendMessage(env, chatId, `🧩 مینی‌اپ فعال شد:
${url}

✅ با این لینک می‌تونی مستقیم وارد مینی‌اپ بشی.`, miniappInlineKeyboard(env, url));
    }
...
async function onStart(env, chatId, from, st, refArg) {
  ...
  const miniUrl = await buildMiniappUrlForUser(env, st.userId);
  if (miniUrl) {
    await tgSetChatMenuButton(env, chatId, miniUrl);
    await tgSendMessage(env, chatId, "🚀 ورود سریع به مینی‌اپ:", miniappInlineKeyboard(env, miniUrl));
  }
...
async function verifyTelegramInitData(initData, botToken, maxAgeSecRaw, lenientRaw) {
  ...
  if (initRaw.startsWith("bot_")) {
    try {
      const raw = initRaw.slice(4);
      const parts = raw.split(".");
      if (parts.length !== 3) return { ok: false, reason: "bot_token_bad_format" };
      const uid = Number(parts[0] || "0");
      const exp = Number(parts[1] || "0");
      const sig = String(parts[2] || "").toLowerCase();
      if (!uid || !exp || !sig) return { ok: false, reason: "bot_token_bad_payload" };
      if (Math.floor(Date.now() / 1000) > exp) return { ok: false, reason: "bot_token_expired" };
      const payload = `${uid}.${exp}`;
      const secret = await hmacSha256Raw(utf8("MiniappBotAuth"), utf8(String(botToken || "miniapp_dev_secret")));
      const expect = await hmacSha256Hex(secret, utf8(payload));
      if (!timingSafeEqualHex(expect, sig)) return { ok: false, reason: "bot_token_bad_sig" };
      return { ok: true, userId: uid, fromLike: { username: "bot_user" } };
    } catch {
      return { ok: false, reason: "bot_token_invalid" };
    }
  }
